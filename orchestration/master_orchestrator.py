...
class MasterOrchestrator:
    ...
    def initialize(
        ...
        PipelineValidator(self.pipeline).validate()

        # -------------------------------------------------
        # Analyze pipeline
        # -------------------------------------------------

        PipelineAnalyzer(self.pipeline).analyze()
    ...
    def run(
        ...
        try:
            ...
            for level in self.pipeline.execution_levels:
                ...
                self.result.add_pipeline(pipeline_result)

            self.result.status = EngineStatus.SUCCESS

        except Exception as exc:
            self.result.status = EngineStatus.FAILED

            self.result.metadata["error"] = str(exc)

            raise

        finally:
            self.result.duration = perf_counter() - timer
    ...
    def _execute_level(
        ...
        engine_results = executor.execute(engine_names)

        pipeline_result.duration = perf_counter() - timer

        for engine_result in engine_results:
            pipeline_result.add_engine(engine_result)

            self.report.add_engine_result(engine_result)

        if pipeline_result.failed_engines == 0:
            pipeline_result.status = EngineStatus.SUCCESS

        else:
            pipeline_result.status = EngineStatus.FAILED

        self.report.add_pipeline_result(pipeline_result)
        ...
        self.result.metadata.update(
            {
                ...
                "total_pipelines": self.result.total_pipelines,
                "successful_pipelines": self.result.successful_pipelines,
                "failed_pipelines": self.result.failed_pipelines,
                "total_engines": self.result.total_engines,
                "successful_engines": self.result.successful_engines,
                "failed_engines": self.result.failed_engines,
                "pipeline_success_rate": self.result.success_rate,
            }
        )

        self.report.set_master_result(self.result)
