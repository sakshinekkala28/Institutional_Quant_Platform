...
class ExecutionEngine:
    ...
    def route(
        ...
        return self.router.route(order)
    ...
    def execute(
        ...
        routed_order = self.route(order)

        return self.execution_model.execute(routed_order)
    ...
    def execute_all(
        ...
        for order in orders:
            reports.append(self.execute(order))
    ...
    def summary(
        ...
        filled = sum(report.is_filled for report in reports)

        partial = sum(report.is_partial for report in reports)

        rejected = sum(report.is_rejected for report in reports)
