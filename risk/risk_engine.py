...
class CovarianceEngine:
    @staticmethod
    def repair_psd(covariance_matrix):
    ...
    def shrink_covariance(covariance_matrix, shrinkage=0.10):
        ...
        return (1 - shrinkage) * covariance_matrix + shrinkage * diag
...
class BetaEngine:
    ...
    def portfolio_beta(portfolio):
        ...
        return (portfolio["Target_Weight"] * portfolio["Beta"]).sum()
...
class RiskContributionEngine:
    @staticmethod
    def calculate(weights, covariance_matrix):
...
class TailRiskEngine:
    ...
    def calculate_cvar(returns, confidence=95):
        ...
        var = TailRiskEngine.calculate_var(returns, confidence)
...
class GovernanceEngine:
    ...
    def effective_holdings(weights):
        ...
        hhi = GovernanceEngine.calculate_hhi(weights)
...
class RiskEngine:
    def evaluate(self, portfolio):
        ...
        report["Portfolio_Beta"] = BetaEngine.portfolio_beta(portfolio)

        report["HHI"] = GovernanceEngine.calculate_hhi(portfolio["Target_Weight"])

        report["Effective_Holdings"] = GovernanceEngine.effective_holdings(
            portfolio["Target_Weight"]
        )

        report["Factor_Exposure"] = FactorExposureEngine.calculate(portfolio)

        print("\n✓ Risk Evaluation Complete")

        print(f"Portfolio Beta: {report['Portfolio_Beta']:.4f}")

        print(f"HHI: {report['HHI']:.4f}")

        print(f"Effective Holdings: {report['Effective_Holdings']:.2f}")
