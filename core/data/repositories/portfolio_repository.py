...
from core.data.repositories.base_repository import BaseRepository
from core.data.validators.dataframe_validator import DataFrameValidator
...
class PortfolioRepository(BaseRepository[Portfolio]):
    ...
    def __init__(self, source: str | Path) -> None:
        ...
        super().__init__(loader=loader, validator=validator)

    # =====================================================
    # TRANSFORM
    # =====================================================

    def _transform(self, dataframe: pd.DataFrame) -> Portfolio:
        """
        ...
        for row in dataframe.itertuples(index=False):
            positions.append(
                PortfolioPosition(
                    ...
                    rank=int(row.Rank),
                    alpha_adjusted=float(row.Alpha_Adjusted),
                    weight=float(row.Weight),
                    last_close=float(row.Last_Close),
                    market_cap=float(row.Market_Cap),
                    adv_20d=float(row.ADV_20D),
                    portfolio_date=str(row.Portfolio_Date),
    ...
    def save(self, portfolio: Portfolio, destination: str | Path) -> None:
        """
        Save portfolio.
        """
    ...
    def top_holdings(self, n: int = 10) -> list[PortfolioPosition]:
        ...
        return self.load().top_holdings(n)
