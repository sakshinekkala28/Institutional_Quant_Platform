...
class DataFrameValidator(BaseValidator):
    ...
    def _validate(self, data: pd.DataFrame) -> list[ValidationIssue]:
        ...
        if not self.allow_empty:
            issues.extend(self.empty_dataframe(data))

        # -----------------------------------------------
        # Required Columns
        # -----------------------------------------------

        issues.extend(self.require_columns(data, self.required_columns))

        # -----------------------------------------------
        # Duplicate Columns
        # -----------------------------------------------

        if not self.allow_duplicate_columns:
            issues.extend(self.duplicate_columns(data))
        ...
        if not self.allow_duplicate_rows:
            ...
            if duplicate_rows > 0:
                issues.append(self.error(f"{duplicate_rows} duplicate rows detected."))
