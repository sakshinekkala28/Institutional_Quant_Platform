    ...
    def _validate(self, data: pd.DataFrame) -> list[ValidationIssue]:
        ...
        for column in self.required_columns:
            if column not in data.columns:
                issues.append(self.error(f"Missing required column: {column}", column))
        ...
        for column in data.columns:
            ...
            if data[column].isna().any():
                issues.append(
                    self.error(f"Column '{column}' contains null values.", column)
                )
        ...
        for column, allowed in self.allowed_values.items():
            ...
            if not invalid.empty:
                issues.append(
                    self.error(f"Column '{column}' contains invalid values.", column)
                )
        ...
        allowed_columns = set(self.required_columns.keys()).union(
            self.optional_columns.keys()
        )

        for column in data.columns:
            if allowed_columns and column not in allowed_columns:
                issues.append(self.warning(f"Unexpected column: {column}", column))
