...
def validate_covariance_matrix(covariance: np.ndarray) -> np.ndarray:
    """
    ...
    if np.isinf(covariance).any():
        raise ValueError("Covariance matrix contains Inf.")
...
def marginal_risk(weights: np.ndarray, covariance: np.ndarray) -> np.ndarray:
    """
    ...
    return (covariance @ weights) / sigma
...
def component_risk(weights: np.ndarray, covariance: np.ndarray) -> np.ndarray:
    """
    Component contribution to risk.
    """

    return weights * marginal_risk(weights, covariance)
...
def risk_contribution(weights: np.ndarray, covariance: np.ndarray) -> np.ndarray:
    ...
    return component_risk(weights, covariance) / sigma
...
def diversification_ratio(weights: np.ndarray, covariance: np.ndarray) -> float:
    ...
    numerator = float(np.dot(weights, asset_volatility))
