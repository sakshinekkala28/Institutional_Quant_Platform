...
def wealth_index(returns: np.ndarray, initial_value: float = 1.0) -> np.ndarray:
    """
    ...
    return initial_value * np.cumprod(1.0 + returns)
...
def running_maximum(wealth: np.ndarray) -> np.ndarray:
    """
    Running peak wealth.
    """
    ...
    return np.maximum.accumulate(wealth)
...
def average_drawdown(returns: np.ndarray) -> float:
    ...
    negative = drawdowns[drawdowns < 0.0]
...
def recovery_period(returns: np.ndarray) -> int:
    """
    ...
    peak_value = peak[peak_index]

    for index in range(peak_index, wealth.size):
...
def underwater_curve(returns: np.ndarray) -> np.ndarray:
    """
...
def is_in_drawdown(returns: np.ndarray) -> bool:
    """
    ...
    return drawdowns[-1] < -RiskConstants.EPSILON
