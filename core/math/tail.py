...
def historical_var(returns: np.ndarray, confidence: float = 0.95) -> float:
    ...
    return abs(float(np.percentile(returns, percentile)))
...
def worst_return(returns: np.ndarray) -> float:
    ...
    return abs(float(np.min(returns)))
...
def tail_volatility(returns: np.ndarray, confidence: float = 0.95) -> float:
    ...
    return standard_deviation(tail)
