...
class AssetReturns:
    ...
    series: dict[str, ReturnSeries] = field(default_factory=dict)
    ...
    def __getitem__(self, symbol: str) -> ReturnSeries:

        try:
            return self.series[symbol]

        except KeyError as exc:
            raise KeyError(f"Unknown asset '{symbol}'.") from exc
    ...
    def get(self, symbol: str) -> ReturnSeries | None:
    ...
    def add(self, symbol: str, returns: ReturnSeries) -> None:

        self.series[symbol] = returns

    def remove(self, symbol: str) -> None:

        self.series.pop(symbol, None)

    def update(self, symbol: str, returns: ReturnSeries) -> None:
    ...
    def validate(self) -> None:
        ...
        for symbol, returns in self.series.items():
            ...
            if not isinstance(returns, ReturnSeries):
                raise TypeError(f"{symbol} must map to ReturnSeries.")
    ...
    def to_dict(self) -> dict[str, list[float]]:
        ...
        return {symbol: returns.to_list() for symbol, returns in self.series.items()}
    ...
    def summary(self) -> dict:
        ...
        return {"asset_count": self.asset_count, "symbols": self.symbols}
    ...
    def __repr__(self) -> str:
        ...
        return f"AssetReturns({self.asset_count} assets)"
