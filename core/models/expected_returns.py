...
class ExpectedReturns:
    ...
    def validate(self) -> None:
        ...
        if len(self.symbols) != len(self.values):
            raise ValueError("Symbols and expected returns size mismatch.")

        if np.isnan(self.values).any():
            raise ValueError("Expected returns contain NaN.")

        if np.isinf(self.values).any():
            raise ValueError("Expected returns contain infinite values.")
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}(Assets={self.size}, Mean={self.mean:.4%})"
