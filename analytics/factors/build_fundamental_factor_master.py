...
OUTPUT_FILE = ROOT_DIR / "data" / "factors" / "fundamental_factor_master.csv"
...
for batch_start in range(0, total, BATCH_SIZE):
    ...
    print(f"\nBatch {batch_start + 1} - {min(batch_start + BATCH_SIZE, total)}")
    ...
    for ticker_symbol in batch:
        try:
            ...
            symbol_row = universe[universe["Yahoo_Symbol"] == ticker_symbol]
            ...
            try:
                ...
                total_assets = safe_float(balance_sheet.loc["Total Assets"].iloc[0])
            ...
            results.append(
                {
                    ...
                    "Factor_Date": datetime.today().strftime("%Y-%m-%d"),
                }
            )

        except Exception:
            print(f"Failed: {ticker_symbol}")
...
print("Columns:", len(factor_df.columns))
