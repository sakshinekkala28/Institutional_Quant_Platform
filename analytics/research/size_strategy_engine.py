...
REPORT_FILE = ROOT / "data" / "logs" / "size_strategy_report.csv"
...
df["MCAP_RANK"] = df["Market_Cap"].rank(pct=True)

df["ADV_RANK"] = df["ADV_20D"].rank(pct=True)

# =========================================================
# SIZE SCORE
# =========================================================

df["Size_Score"] = 0.80 * df["MCAP_RANK"] + 0.20 * df["ADV_RANK"]

# =========================================================
# PORTFOLIO
# =========================================================

portfolio = df.sort_values("Size_Score", ascending=False).head(TOP_N).copy()
...
print("🏁 SIZE STRATEGY COMPLETE")

print("=" * 70)

print(f"Securities : {len(portfolio)}")

print(f"Average Score : {portfolio['Size_Score'].mean():.4f}")
