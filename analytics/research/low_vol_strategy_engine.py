...
REPORT_FILE = ROOT / "data" / "logs" / "low_vol_strategy_report.csv"
...
df["VOL20_RANK"] = 1 - (df["Volatility_20D"].rank(pct=True))

df["VOL60_RANK"] = 1 - (df["Volatility_60D"].rank(pct=True))

df["ATR_RANK"] = 1 - (df["ATR_14"].rank(pct=True))

df["DD_RANK"] = 1 - (df["Max_Drawdown_252D"].rank(pct=True))
...
portfolio = df.sort_values("LowVol_Score", ascending=False).head(TOP_N).copy()
...
print("🏁 LOW VOL STRATEGY COMPLETE")

print("=" * 70)

print(f"Securities : {len(portfolio)}")

print(f"Average Score : {portfolio['LowVol_Score'].mean():.4f}")
