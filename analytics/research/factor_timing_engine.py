...
REPORT_FILE = ROOT / "data" / "logs" / "factor_timing_report.csv"
...
if regimes.empty:
    raise ValueError("Regime file empty")
...
regimes = regimes.rename(columns={"Date": "Snapshot_Date"})
...
if "Last_Close" not in factors.columns:
    raise ValueError("Last_Close required")
...
factors["Forward_Return"] = (
    factors.groupby("Symbol")["Last_Close"].shift(-1) / factors["Last_Close"] - 1
)
...
merged = pd.merge_asof(
    factors.sort_values("Snapshot_Date"),
    regimes.sort_values("Snapshot_Date"),
    on="Snapshot_Date",
    direction="backward",
)
...
factor_columns = [f for f in candidate_factors if f in merged.columns]
...
for factor in factor_columns:
    for regime in sorted(merged["Regime"].dropna().unique()):
        tmp = merged[merged["Regime"] == regime][
            [
                factor,
                "Forward_Return",
            ]
...
print("\nUnique Snapshot Dates:", merged["Snapshot_Date"].nunique())

print("\nForward Return Count:", merged["Forward_Return"].notna().sum())
...
summary = timing.groupby("Factor").agg(
    {
        "IC": [
            "mean",
            "std",
...
summary["Timing_Score"] = summary["Best_IC"] - summary["Worst_IC"]
...
timing.to_csv(
    OUTPUT_DIR / "factor_timing.csv",
    index=False,
)

summary.to_csv(
    OUTPUT_DIR / "factor_timing_summary.csv",
    index=False,
)

rankings.to_csv(
    OUTPUT_DIR / "factor_regime_rankings.csv",
    index=False,
)

heatmap.to_csv(
    OUTPUT_DIR / "factor_timing_heatmap.csv",
    index=False,
)
...
print("🏁 FACTOR TIMING ENGINE COMPLETE")
...
print(f"Regime IC        : {best['Best_Regime_IC']:.4f}")
