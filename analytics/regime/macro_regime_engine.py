...
from analytics.regime.macro_repository import MacroLatestValueExtractor, MacroRepository
...
MACRO_HISTORY_FILE = REGIME_DIR / "macro_regime_history.csv"

REGIME_DIR.mkdir(parents=True, exist_ok=True)

OUTPUT_FILE = REGIME_DIR / "macro_regime_dashboard.csv"


class MacroSignalEngine:
    @staticmethod
    def build(macro_values):
        ...
        signals["US10Y"] = max(0, min(100, 100 - (us10y * 10)))

        india10y = macro_values["INDIA10Y"]

        signals["INDIA10Y"] = max(0, min(100, 100 - (india10y * 10)))

        cpi = macro_values["CPI"]

        signals["CPI"] = max(0, min(100, 100 - (cpi * 12)))

        wpi = macro_values["WPI"]

        signals["WPI"] = max(0, min(100, 100 - (wpi * 15)))

        usdinr = macro_values["USDINR"]

        signals["USDINR"] = max(0, min(100, 120 - usdinr))

        vix = macro_values["VIX"]

        signals["VIX"] = max(0, min(100, 100 - (vix * 3)))
...
class MacroRegimeClassifier:
    ...
    def classify(score):
        ...
        if score >= 80:
            return ("STRONG_RISK_ON", "AGGRESSIVE_OVERWEIGHT")

        if score >= 65:
            return ("RISK_ON", "OVERWEIGHT")

        if score >= 50:
            return ("NEUTRAL", "MARKET_WEIGHT")

        if score >= 35:
            return ("RISK_OFF", "UNDERWEIGHT")
...
class MacroRiskEngine:
    ...
    def build(signals):
        ...
        logger.info("Calculating Macro Risk")

        average_signal = np.mean(list(signals.values()))
...
class MacroHistoryEngine:
    ...
    def archive(score, regime, risk):
        ...
        if MACRO_HISTORY_FILE.exists():
            existing = pd.read_csv(MACRO_HISTORY_FILE)

            history_record = pd.concat([existing, history_record], ignore_index=True)

        history_record.to_csv(MACRO_HISTORY_FILE, index=False)
...
class MacroConfidenceEngine:
    ...
    def calculate(signals):
        ...
        dispersion = np.std(list(signals.values()))
...
class MacroRegimeExporter:
    ...
    def export(dashboard):
        ...
        logger.info("Exporting Macro Dashboard")
...
def run_example():
    ...
    logger.info("Starting Macro Regime Engine")

    logger.info("Validating Macro Repository")

    macro_data = MacroRepository.load()

    macro_values = MacroLatestValueExtractor.extract(macro_data)

    signals = MacroSignalEngine.build(macro_values)
    ...
    MacroHistoryEngine.archive(score, regime, risk)

    print()

    print("=" * 80)

    print("MACRO REGIME DASHBOARD")
