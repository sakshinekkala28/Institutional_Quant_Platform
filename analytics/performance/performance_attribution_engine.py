...
EQUITY_FILE = ROOT / "data" / "backtests" / "equity_curve.csv"
...
REPORT_FILE = ROOT / "data" / "logs" / "performance_attribution_report.csv"
...
if abs(weight_sum - 1.0) > 0.01:
    print(f"⚠ Normalizing weights (sum={weight_sum:.4f})")
...
if "Benchmark_Return" in equity.columns:
    equity["Benchmark_Return"] = pd.to_numeric(
        equity["Benchmark_Return"], errors="coerce"
    )

if "Active_Return" in equity.columns:
    equity["Active_Return"] = pd.to_numeric(equity["Active_Return"], errors="coerce")

equity = equity.dropna(subset=["Portfolio_Return"])

# =========================================================
# PERFORMANCE STATISTICS
# =========================================================

portfolio_returns = equity["Portfolio_Return"]

if "Benchmark_Return" in equity.columns:
    benchmark_returns = equity["Benchmark_Return"].dropna()

    benchmark_coverage = len(benchmark_returns) / len(equity)

    print(f"Benchmark Coverage : {benchmark_coverage:.2%}")

    if benchmark_coverage < 0.95:
        print("⚠ Benchmark coverage below 95%")
...
if pd.notna(benchmark_total_return):
...
if active_returns is not None:
    ...
    information_ratio = (active_returns.mean() * TRADING_DAYS) / max(
        tracking_error, 1e-9
    )
...
sharpe_ratio = annual_return / max(annual_volatility, 1e-9)
...
if benchmark_returns is not None:
    ...
    alpha = (
        aligned["Portfolio"].mean() - beta * aligned["Benchmark"].mean()
    ) * TRADING_DAYS

    correlation = np.corrcoef(aligned["Portfolio"], aligned["Benchmark"])[0, 1]
...
factors = factors[factors["Snapshot_Date"] == latest_date]
...
factor_attribution = pd.DataFrame(factor_records)

if not factor_attribution.empty:
    total = max(
        factor_attribution["Contribution"].abs().sum(),
        1e-9,
    )

    factor_attribution["Contribution_%"] = (
        factor_attribution["Contribution"].abs() / total * 100
    )
...
if "Sector" not in merged.columns:
    sector_attribution = pd.DataFrame(
        {
            "Sector": ["UNKNOWN"],
            "Portfolio_Weight": [merged["Weight"].sum()],
            "Sector_Score": [0],
            "Contribution": [0],
        }
    )
    ...
    if "Alpha_Adjusted" in merged.columns:
        sector_attribution = (
            ...
            .agg(
                ...
                Sector_Score=("Alpha_Adjusted", "mean"),
            )
            .reset_index()
        )
        ...
        sector_attribution = (
            ...
            .agg(
                Portfolio_Weight=("Weight", "sum"),
                Sector_Score=("Weight", "count"),
            )
            .reset_index()
        )

    sector_attribution["Contribution"] = (
        sector_attribution["Portfolio_Weight"] * sector_attribution["Sector_Score"]
    )
...
AUDIT_FILE = OUTPUT_DIR / "performance_audit.csv"
...
dashboard = pd.DataFrame(
    {
        ...
        "Value": [
            ...
            beta,
            r_squared,
            len(factor_attribution),
            len(sector_attribution),
            ENGINE_VERSION,
        ],
    }
)
...
benchmark_display = (
    f"{benchmark_total_return:.2%}" if pd.notna(benchmark_total_return) else "N/A"
)
...
print(f"Benchmark Return : {benchmark_total_return:.2%}")

print(f"excess_return            : {excess_return:.2%}")
