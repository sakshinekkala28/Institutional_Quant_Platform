...
INPUT_FILE = ROOT_DIR / "data" / "raw" / "security_master.csv"

OUTPUT_FILE = ROOT_DIR / "data" / "risk" / "beta_master.csv"
...
required_cols = ["Symbol", "Yahoo_Symbol"]
...
benchmark = yf.download(
    BENCHMARK, period=LOOKBACK_PERIOD, auto_adjust=True, progress=False
)
...
print("✓ Stock Universe Downloaded")
...
def calculate_beta(stock_returns, benchmark_returns):
    ...
    beta = stock.cov(bench) / bench_var
...
for i, symbol in enumerate(symbols, start=1):
    try:
        ...
        raw_beta = calculate_beta(returns, benchmark_returns)

        # =====================================
        # BAYESIAN SHRINKAGE
        # =====================================

        beta = (1 - SHRINKAGE_WEIGHT) * raw_beta + SHRINKAGE_WEIGHT * 1.0
...
beta_df["Beta"] = (
    beta_df["Beta"].replace([np.inf, -np.inf], np.nan).fillna(DEFAULT_BETA)
)

beta_df["Beta"] = beta_df["Beta"].clip(lower=BETA_FLOOR, upper=BETA_CAP)
...
print(beta_df.sort_values("Beta").head(10))

print("\nFailed Symbols:", len(failed_symbols))

if failed_symbols:
    pd.DataFrame({"Yahoo_Symbol": failed_symbols}).to_csv(
        ROOT_DIR / "data" / "risk" / "beta_failures.csv", index=False
    )
# =========================================================
# SAVE
# =========================================================
...
print(f"Total Stocks: {len(beta_df)}")
