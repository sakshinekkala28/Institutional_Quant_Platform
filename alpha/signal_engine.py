...
class PathManager:
    ...
    def fundamental_factor_file(self):
        return self.root / "data/factors/fundamental_factor_master.csv"
    ...
    def factor_exposure_file(self):
        return self.root / "data/risk/factor_exposures.csv"
...
class DataValidator:
    ...
    def coverage_report(df):
        ...
        for col in df.columns:
            report.append(
                {"Column": col, "Coverage_%": round(df[col].notna().mean() * 100, 2)}
            )
    ...
    def duplicate_symbols(df):
        ...
        return df[df["Symbol"].duplicated(keep=False)]
...
class DataStandardizer:
    ...
    def standardize_master_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        mappings = [
            ...
            ("Market_Cap_x", "Market_Cap_y", "Market_Cap"),
            ("ADV_20D_x", "ADV_20D_y", "ADV_20D"),
            ("Beta_x", "Beta_y", "Beta"),
        ]

        for left, right, final in mappings:
            if left in df.columns:
                df[final] = df[left].fillna(df.get(right))
    ...
    def standardize_security_master(security_master: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            ...
            and "ADV_20D" not in security_master.columns
        ):
...
class FactorNormalizer:
    ...
    @staticmethod
    def winsorized_rank(series: pd.Series, lower=0.01, upper=0.99) -> pd.Series:
...
class SignalFreshnessEngine:
    ...
    def apply(df: pd.DataFrame, half_life_days: int = 90) -> pd.DataFrame:
        ...
        age_days = (pd.Timestamp.today() - df["Signal_Date"]).dt.days

        age_days = age_days.fillna(half_life_days)

        df["Freshness_Factor"] = np.exp(-age_days / half_life_days)
...
class CoverageAnalytics:
    ...
    def print_report(target: pd.DataFrame):
        ...
        report = CoverageAnalytics.report(target)
...
class FactorEngine:
    ...
    def build_momentum_factor(self):
        ...
        available = [c for c in momentum_cols if c in self.df.columns]

        if available:
            self.df["Momentum_Factor"] = self.df[available].rank(pct=True).mean(axis=1)
    ...
    def build_quality_factor(self):
        ...
        available = [
            ...
            if c in self.df.columns and self.df[c].notna().sum() > 20
        ]

        if available:
            self.df["Quality_Factor"] = self.df[available].rank(pct=True).mean(axis=1)
    ...
    def build_value_factor(self):
        ...
        valid_pe = self.df["PE"].where(self.df["PE"] > 0)
        ...
        self.df["Value_Factor"] = inverse_pe.rank(pct=True).fillna(0.50)
    ...
    def build_growth_factor(self):
        ...
        available = [c for c in growth_cols if c in self.df.columns]

        if available:
            self.df["Growth_Factor"] = self.df[available].rank(pct=True).mean(axis=1)
    ...
    def build_liquidity_factor(self):
        ...
        self.df["Liquidity_Factor"] = self.df["ADV_20D"].rank(pct=True).fillna(0.50)
    ...
    def build_low_vol_factor(self):
        ...
        self.df["LowVol_Factor"] = 1 - self.df["Volatility_252D"].rank(pct=True)
    ...
    def build_signal_factor(self):
        ...
        self.df["Signal_Factor"] = self.df["Signal_Score"].rank(pct=True).fillna(0.50)
    ...
    def report(cls, df):
        ...
        for factor in cls.FACTORS:
            ...
            records.append(
                {
                    "Factor": factor,
                    "Coverage_%": round(df[factor].notna().mean() * 100, 2),
                    "Unique_Values": df[factor].nunique(),
                    "Std_Dev": round(df[factor].std(), 4),
                }
            )
...
class FactorCorrelationEngine:
    ...
    def correlation_matrix(cls, df):
        ...
        available = [c for c in cls.FACTORS if c in df.columns]
...
class FactorDiagnostics:
    ...
    def run(df):
        ...
        health = FactorHealthEngine.report(df)

        print(health)

        print("\n📈 FACTOR CORRELATION")

        corr = FactorCorrelationEngine.correlation_matrix(df)

        print(corr)

        return {"health": health, "correlation": corr}
...
class RegimeEngine:
    ...
    @classmethod
    def get_weights(cls, regime_name):
        ...
        return cls.REGIME_WEIGHTS["SIDEWAYS"]
...
class AlphaEngine:
    ...
    def build_composite_alpha(df, regime_name="SIDEWAYS"):
        ...
        weights = RegimeEngine.get_weights(regime_name)

        df["Composite_Alpha"] = (
            weights["Signal"] * df["Signal_Factor"]
            + weights["Momentum"] * df["Momentum_Factor"]
            + weights["Quality"] * df["Quality_Factor"]
            + weights["Value"] * df["Value_Factor"]
            + weights["Growth"] * df["Growth_Factor"]
            + weights["Liquidity"] * df["Liquidity_Factor"]
            + weights["LowVol"] * df["LowVol_Factor"]
        )

        df["Composite_Alpha"] = df["Composite_Alpha"].rank(pct=True)
...
class SectorAlphaEngine:
    ...
    def build(df):
        ...
        sector_alpha = df.groupby("Sector")["Composite_Alpha"].mean()

        sector_alpha = sector_alpha.rank(pct=True)

        df["Sector_Alpha"] = df["Sector"].map(sector_alpha).fillna(0.50)
...
class UniverseBuilder:
    ...
    def build(df, config):
        ...
        alpha_cutoff = df["Composite_Alpha"].quantile(config.ALPHA_CUTOFF_QUANTILE)

        df = df[df["Composite_Alpha"] >= alpha_cutoff].copy()

        if "ADV_20D" in df.columns:
            min_adv = df["ADV_20D"].quantile(config.MIN_LIQUIDITY_PERCENTILE)

            df = df[df["ADV_20D"] >= min_adv].copy()
...
class SelectionScoreEngine:
    ...
    def build(df):
        ...
        sector_count = df["Sector"].map(df["Sector"].value_counts())

        diversification_bonus = 1 / np.sqrt(sector_count)

        df["Selection_Score"] = (
            df["Composite_Alpha"]
            * (0.80 + 0.40 * df["Sector_Alpha"])
...
class SignalEngine:
    ...
    def load_data(self):
        ...
        return {
            ...
            "portfolio": self.loader.load_csv(self.paths.portfolio_file),
            "security_master": self.loader.load_csv(self.paths.security_master_file),
            "beta": self.loader.load_csv(self.paths.beta_file),
            "volatility": self.loader.load_csv(self.paths.volatility_file),
            "factor_master": self.loader.load_csv(self.paths.factor_file),
            "fundamental_factor": self.loader.load_csv(
    ...
    def build_alpha_universe(self, data):
        ...
        security_master = DataStandardizer.standardize_security_master(
            data["security_master"]
        )

        target = signals.merge(security_master, on="Symbol", how="left")

        if len(data["factor_master"]):
            target = target.merge(data["factor_master"], on="Symbol", how="left")

        if len(data["fundamental_factor"]):
            target = target.merge(data["fundamental_factor"], on="Symbol", how="left")

        target = DataStandardizer.standardize_master_columns(target)

        if len(data["volatility"]) > 0:
            vol_cols = [c for c in data["volatility"].columns if "vol" in c.lower()]

            if vol_cols:
                target = target.merge(
                    data["volatility"][["Symbol", vol_cols[0]]], on="Symbol", how="left"
                )

                target.rename(columns={vol_cols[0]: "Volatility_252D"}, inplace=True)

        if len(data["beta"]) > 0:
            target = target.merge(data["beta"], on="Symbol", how="left")

        target["Beta"] = target["Beta"].fillna(self.config.DEFAULT_BETA)

        target["Volatility_252D"] = target["Volatility_252D"].fillna(
            self.config.DEFAULT_VOLATILITY
        )

        target = SignalFreshnessEngine.apply(target, self.config.SIGNAL_HALF_LIFE_DAYS)

        target = FactorEngine(target).run()

        regime_name = "SIDEWAYS"

        if len(data["regime"]) > 0:
            regime_name = data["regime"].iloc[-1].get("Regime", "SIDEWAYS")

        target = AlphaEngine.build_composite_alpha(target, regime_name)

        target = SectorAlphaEngine.build(target)

        target = SelectionScoreEngine.build(target)

        target = UniverseBuilder.build(target, self.config)

        CoverageAnalytics.print_report(target)

        FactorDiagnostics.run(target)

        ExposureExporter.export(target, self.paths.factor_exposure_file)

        print("\n✓ Alpha Universe Built")

        print(f"Universe Size: {len(target):,}")
