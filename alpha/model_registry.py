...
class ModelPerformanceTracker:
    ...
    def record(self, model_name, version, metrics):
    ...
    def get(self, model_name, version):
        ...
        return self.performance.get((model_name, version))
    ...
    def leaderboard(self):
        ...
        for key, metrics in self.performance.items():
            rows.append({"Model": key[0], "Version": key[1], **metrics})
...
class ModelPromotionEngine:
    ...
    def promote(self, model_name, version):
        ...
        self.production_model = {"Model": model_name, "Version": version}
...
class ModelComparisonEngine:
    @staticmethod
    def compare(performance_df, metric="Sharpe"):

        return performance_df.sort_values(metric, ascending=False)
...
class ModelRegistryEngine:
    def __init__(self):
        ...
        self.promotion = ModelPromotionEngine()

    def register_model(self, model_name, version, owner, description):
    ...
    def record_performance(self, model_name, version, metrics):

        self.performance.record(model_name, version, metrics)

    def promote_model(self, model_name, version):

        self.promotion.promote(model_name, version)

    def leaderboard(self):

        return self.performance.leaderboard()
